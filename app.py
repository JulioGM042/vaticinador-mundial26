...
def procesar_historico_equipos():
    # --- DESCOMPRESIÓN AL VUELO ---
    # Si el .db no existe en el servidor, lo extraemos del .zip
    if not os.path.exists('mundial2026.db'):
        with zipfile.ZipFile('mundial2026.zip', 'r') as zip_ref:
            zip_ref.extractall('.')
            
    # Conexión segura a la base descomprimida
...
if local == visitante:
    ...
    if hist_local.empty or hist_visit.empty:
        ...
        # Dibujar los estados de forma previos
