...
# ==============================================================================
# 1. DICCIONARIO DE LOS 48 CLASIFICADOS
# ==============================================================================
DICT_EQUIPOS_2026 = {
...
# Perfil Base de Emergencia (Valores neutrales para rellenar vacíos parciales o totales)
PERFIL_BASE_MUNDIAL = {
...
# ==============================================================================
# 2. PROCESAMIENTO DE DATOS CON EXTRACCIÓN EN ZIP
# ==============================================================================
@st.cache_resource
def cargar_modelos():
...
if local_es == visitante_es:
    ...
    def extraer_metricas_equipo(meta_equipo):
        ...
        base = PERFIL_BASE_MUNDIAL
        
        # 1. Intento de extracción por Rolling
        ...
        # 2. Intento de extracción por Promedio Histórico Bruto
            ...
            res = {
                ...
                'origen': "Promedio Histórico Total"
            }
        # 3. Asignación del Perfil de Emergencia
        else:
            res = {
                'xg': base['xg_ataque_roll_3'], 'xgot': base['xgot_ataque_roll_3'],
                'pos': base['posesion_roll_3'], 'tiros': base['tiros_arco_roll_3'],
                'corners': base['corners_roll_3'], 'origen': "Perfil Base Asignado"
            }
            
        # --- ESCUDO ANTI-BUG: Control estricto campo por campo ---
        if pd.isna(res['xg']) or res['xg'] is None: res['xg'] = base['xg_ataque_roll_3']
        if pd.isna(res['xgot']) or res['xgot'] is None: res['xgot'] = base['xgot_ataque_roll_3']
        if pd.isna(res['pos']) or res['pos'] is None: res['pos'] = base['posesion_roll_3']
        if pd.isna(res['tiros']) or res['tiros'] is None: res['tiros'] = base['tiros_arco_roll_3']
        if pd.isna(res['corners']) or res['corners'] is None: res['corners'] = base['corners_roll_3']
    ...
    with c_l:
        ...
        st.caption(f"🔹 Corners: {medidas_l['corners']:.1f}")
    ...
    with c_v:
        ...
        st.caption(f"🔹 Corners: {medidas_v['corners']:.1f}")
